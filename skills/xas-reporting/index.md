...
ACTIVE ONLY (2026-09-03): all 7 inactive classifications are omitted, along with 2 entities
left with none. That includes `VGR` and `LeaseContract`, which were kept and marked until this
date because between them they hold 39 live job cards — a breakdown by type now reports those
39 under no type at all, which is the accepted cost of an active-only list. So a classification
being absent here is NOT evidence that no card carries it, and `index_lookup` on the MCP server
does not filter this way — it can still return an inactive classification.
