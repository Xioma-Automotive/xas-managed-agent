...
TTL is evaluated against a *current week*, not real time, for the same reason.
"""
...
from .synth_data import parse_week_label
...
class LedgerEntry:
    ...
    ttl: Optional[str] = None      # week label, e.g. "2026-W40"; None = permanent
...
def _entry_active(entry: LedgerEntry, current_week: int) -> bool:
    """A TTL'd entry is skipped once the current week passes its expiry, so a
    one-off nudge can't silently distort a later cycle (§7)."""
    if entry.ttl is None:
        return True
    return current_week <= parse_week_label(entry.ttl)
...
class Ledger:
    ...
    # --- persistence ---------------------------------------------------------
    @classmethod
    def load(cls, path: Optional[str | Path]) -> "Ledger":
        if path is None:
    ...
    # --- replay --------------------------------------------------------------
    def replay(self, current_week: int) -> dict:
        ...
        for e in self.entries:
            if not _entry_active(e, current_week):
                continue
            ov = e.override or {}
    ...
    # --- attribution (§7) ----------------------------------------------------
    def who_touched(self, order_id: int, current_week: int) -> list[str]:
        ...
        for e in self.entries:
            if not _entry_active(e, current_week):
                continue
            ov = e.override or {}
            hits = [p for p in ov.get("pins", []) if int(p.get("order", -1)) == order_id]
            hits += [f for f in ov.get("forbid", []) if int(f.get("order", -1)) == order_id]
            for h in hits:
                trail.append(
                    f"turn {e.turn} ({e.author}): {h.get('action', 'pin')} on {order_id}"
                )
        return trail
