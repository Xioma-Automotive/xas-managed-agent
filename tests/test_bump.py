...
def _snapshot() -> Snapshot:
    ...
    return Snapshot(
        orders=[_order("SO-HI-1", "CUST-001", "A"), _order("SO-LO-1", "CUST-002", "C")],
        units=[
...
def test_without_authorization_no_bump_high_priority_stays_late():
    ...
    assert result.plan["SO-LO-1"] == "VEH-LO-GOOD", "untouched row must not move uninvited"
    ...
    assert any(c["row"] == "SO-LO-1" for c in cands)


def test_authorized_bump_rescues_the_disrupted_row():
    snap = _snapshot()
    result = solve(snap, {"bump": {"orders": ["SO-LO-1"]}}, lam=0)
    units = snap.unit_by_id()
