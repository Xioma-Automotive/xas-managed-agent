...
**Architecture:** The solver's reports are already written for the planner, but `web.py:_render` drops every built-in tool result, so the agent retypes them. Add a marked span: report text printed inside two sentinel lines is forwarded by `web.py` to the planner's screen and rendered as markdown. The agent then adds judgment in a sentence instead of re-typesetting data.

A tool result is the ONLY way a report leaves the sandbox: a file the agent writes is not retrievable (Task 4), and egress is MCP-only so it cannot upload one. So one copy in the conversation is the floor, and this change removes the second.
...
**One copy is the floor**, not zero: the report can only leave the sandbox on a tool result, and a tool result is in the agent's context by definition. Task 5 aimed at zero via a file channel; Task 4 established there is none, so it was dropped.
...
### Task 4: Whether an agent-written file can be retrieved — ANSWERED 2026-08-27

**No.** Settled without a live probe, from three sessions that were still `idle`
(alive) and had each written `/workspace/snapshot.json` and `/workspace/plan.json`.
All three list only the two payloads `web.py` mounted; the mounts themselves
refuse to download (`file_not_downloadable`). The container's filesystem and the
session's file store are different stores, and the agent cannot bridge them
either — egress is MCP-only, so it cannot reach the Files API to upload.

Full record, including the two pieces of dead code this exposes:
`docs/planner-channel-file-probe.md`.

- [x] **Answered.** No session was created and nothing was spent.

---

### Task 5: Take the table out of the agent's context — DROPPED

Depended on Task 4. Without a file channel the planner-facing table has to ride
out of the sandbox on a tool result, so it crosses the conversation **once** —
which Tasks 1-3 already achieve. `plan_summary` would earn nothing while the
table is still in the agent's context: the agent would simply hold both.

Not deferred, dropped. Re-open it only if `docs/planner-channel-file-probe.md` is
re-run and the answer changes.

---

## Self-review

**Spec coverage.** Fix 1 (show the write-up, stop the retype) = Tasks 1, 2, 3 — done.
Fix 2 (agent gets a summary) = Task 5, gated by Task 4 — Task 4 came back negative,
so Task 5 was dropped rather than built. The reason is recorded in both places.
...
1. ~~Can an agent-written file be retrieved at all?~~ **Answered: no.** See
   `docs/planner-channel-file-probe.md`. The table crosses the conversation once.
