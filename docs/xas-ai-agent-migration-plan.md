...
| D-3 | Embeddings / the terminology index | The index is the trickiest subsystem — see the dedicated **`docs/xas-index-adaptation-plan.md`**. Summary: mount the tenant index as a file, flatten it once into a compact bidirectional phrasebook, query it in-sandbox with a progressively smarter matcher (grep → SQLite/FTS5 → local embeddings), and keep Titan/MCP search only as an eval-driven escape hatch. (Design B fallback keeps Titan host-side in the worker.) |
